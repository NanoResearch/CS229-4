...
% !!!!!!Here you should use a1&a2 with bias
    a1i = Xb(i, :);
    a2i = a2b(i, :);
    z2i = z2(i, :);
    a3i = a3(i, :);
%         For each output unit k in layer 3 (the output layer), set ?(3) = (a(3) ? yk),
    delta3 = (a3i - Y(i, :))';
%         For the hidden layer l = 2, set ?(2) = ?(2)'*?(3).?g'(z(2))
    z2i = [0 z2i]';
    delta2 = Theta2'*delta3.*sigmoidGradient(z2i);
...
temp1 = Theta1(:, 2:end);
temp2 = Theta2(:, 2:end);
J = J + (sum(sum(temp1.^2)) + sum(sum(temp2.^2)))*lambda/2/m;
